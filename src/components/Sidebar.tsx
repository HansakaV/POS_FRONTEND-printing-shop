...
const Sidebar: React.FC<SidebarProps> = ({ className }) => {
  ...
  const navItems: { name: string; path: string; icon: any; color: string }[] = [
  ...
  if (user?.role === "admin") {
    navItems.push({ name: 'Management', path: '/dashboard/management', icon: Bell, color: 'from-pink-400 to-pink-600' });
  }
  ...
  return (
    <div className={`fixed md:relative left-0 top-0 h-screen w-72 md:w-72 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white shadow-2xl z-50 overflow-hidden ${className}`}>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
              ...
              return (
                <button
                  ...
                  className={`relative flex items-center w-full px-4 py-3 rounded-xl text-left font-medium transition-all duration-300 group overflow-hidden animate-slideIn
                    ${isActive ? 'bg-white/10 text-white shadow-lg' : 'hover:bg-white/5 text-slate-300 hover:text-white'}`}>
                  
                  {isActive && <div className={`absolute inset-0 bg-gradient-to-r ${item.color} opacity-20 rounded-xl`} />}
                  <div className={`relative z-10 w-9 h-9 rounded-lg flex items-center justify-center mr-3 transition-all duration-300
                    ${isActive ? `bg-gradient-to-r ${item.color}` : 'bg-white/5 group-hover:bg-white/10'}`}>
                    <Icon size={18} />
                  </div>
                  <span className="relative z-10">{item.name}</span>
                  {!isActive && <div className="absolute right-2 opacity-0 group-hover:opacity-100 transition-opacity"><div className="w-1.5 h-1.5 bg-white rounded-full"></div></div>}
                </button>
              );
            })}
          </nav>
          ...
          <div className="px-4 pb-4 animate-slideIn" style={{ animationDelay: '0.5s' }}>
            ...
            <div className="bg-white/5 backdrop-blur-sm rounded-lg p-4 border border-white/10 hover:bg-white/10 transition-all duration-300 animate-scaleIn">
              <p className="text-sm text-slate-200 mb-2"><strong>Developer:</strong> Mahesh Hansaka</p>
              <p className="text-sm text-slate-200 mb-2"><strong>Email:</strong> <a href="mailto:hasamahesh01@gmail.com" className="underline hover:text-blue-400">hasamahesh01@gmail.com</a></p>
              <p className="text-sm text-slate-200 mb-2"><strong>Phone:</strong> <a href="tel:+94710356244" className="underline hover:text-blue-400">+94 71 035 6244</a></p>
              <p className="text-sm text-slate-200"><strong>Website:</strong> <a href="https://neon-mahesh-portfolio.vercel.app" target="_blank" className="underline hover:text-blue-400">Mahesh Hansaka</a></p>
            </div>
          </div>
